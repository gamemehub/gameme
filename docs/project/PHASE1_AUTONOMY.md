...
## 実行順序（GPT確定版 2026-07-02）
**② Chairman設定 → ① Mac mini 24hエージェント → ③ 1エリア縦串**。
- **禁止：基盤だけ作る／Runnerだけ／Agentだけ／CIだけ磨く。** 必ず**CH01の1エリアを実際に作らせながら**基盤を育てる（ドッグフード）。
- 7月末の達成目標は3つ：**(a) AIが24h自律開発できる (b) Godot CIが常時動作 (c) CH01 1エリア提出品質**。

## 原則（CTO補足）
- **最小で効く基盤に絞る。** 基盤を真空で磨かない。縦串を作りながら基盤を回して検証。
- スコープ管理が最大リスク。7月に「エンジン改善/システム追加/アート/演出/AI基盤」を同時多発させない。

## Phase1 完了条件（無人でここまで到達）
- [ ] Ready Issue を取得できる
- [ ] 自動ブランチ作成
- [ ] 実装
- [ ] Commit
- [ ] PR 作成（auto-pr）
- [ ] **Godot CI (godot-check) Green**
- [ ] 自動レビュー（Claude/Copilot）
- [ ] 人間レビュー待ちで停止
→ ここまで無人で回ったら Phase2（1エリア提出品質）へリソース集中。

## Supervisor（監視エージェント・GPT追加）
エージェントを監視するエージェントを必ず置く。役割：
- Agent停止 → 再起動 / 無限ループ検知 / API異常検知 / CI失敗率監視 / トークン使用量監視 / 24h以上停止で通知。
- 実体：`tools/autonomy/supervisor.sh`（launchd 常駐）。
