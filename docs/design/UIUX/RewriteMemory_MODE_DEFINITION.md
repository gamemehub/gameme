...
### M03 探索会話モード
- **用途**: 探索中のすべての会話（NPC会話・調査・独り言・システム通知・選択肢）
- **特徴**: 下部会話枠を基本とし、サブモード A〜E を持つ
  - **M03A 探索会話**: PortraitSmall・名前あり・探索画面を保持
  - **M03B イベント会話**: PortraitFull（立ち絵）・正面表示
  - **M03C 独り言 / M03D システム通知**: Portrait・名前なし
  - **M03E 選択肢**: ChoiceList
- **責務分離**: M03＝会話。M04＝記憶・白層化・特殊演出（M04 で通常会話は扱わない）
